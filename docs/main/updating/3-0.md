...
プラグインを Capacitor の新しいバージョンにアップグレードしたいと考えているプラグイン作者の方は、 [Upgrade Guide for Capacitor Plugins](/main/updating/plugins/3-0.md) をご覧ください。
...
- **Permissions**: コアチームはこの集中型のアプローチに代わるものを実装しました。コミュニティのプラグインもこれを採用することができます（[new Permissions API](/plugins/creating-plugins/web-guide.md#permissions) を参照）
