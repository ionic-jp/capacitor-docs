...
Capacitor CLI の最新版をプロジェクトにインストールします：
...
スキームを変更することは、アプリケーションを別のドメインで出荷することと同じであり、Cookieやlocalstorageなどに保存されたデータにはアクセスできなくなることを意味します。この変更に伴うデータの損失を避けるために、たとえ現在のデフォルトが`http`であったとしても、今すぐスキームを設定すべきです。
