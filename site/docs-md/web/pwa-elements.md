...
<p class="intro"><code>Camera</code> や <code>Toast</code> といった一部のCapacitorプラグインは、, ネイティブに実行されていないときに使用できるWebベースのUIがあります。たとえば、<code>Camera.getPhoto()</code>を実行すると、Webまたはelectron上での実行時に応答性の高い写真撮影がロードされます。</p>
